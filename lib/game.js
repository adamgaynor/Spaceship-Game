  ...
  Game.prototype.checkShipCollisions = function () {
    ...
    self.asteroids.forEach(function (asteroid) {
      if (Asteroids.Util.checkCollision(asteroid.asteroid, self.ship.spaceship)) {
        self.stage.removeChild(self.ship.spaceship);
        self.stage.addChild(self.ship.explode());
        self.stage.off("stagemousemove", self.moveListener);
        self.stage.off("stagemousedown", self.clickListener);
